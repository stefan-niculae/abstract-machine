...
var Assign, Branch, Break, Cond, Continue, Exit, Expr, If, Loop, Save, Seq, Skip, ValOf, While, assertType,
  slice = [].slice;

assertType = function() {
  var i, len, obj, type, types;
  obj = arguments[0], types = 2 <= arguments.length ? slice.call(arguments, 1) : [];
...
Assign = (function() {
  function Assign(arg) {
    this["var"] = arg["var"], this.value = arg.value;
    assertType(this["var"], String);
    assertType(this.value, Expr, Number, ValOf);
  }
...
Seq = (function() {
  function Seq(arg) {
    this.s1 = arg.s1, this.s2 = arg.s2;
    assertType(this.s1, If, While, Seq, Assign, Skip, Break, Continue, Exit);
    assertType(this.s2, If, While, Seq, Assign, Skip, Break, Continue, Exit);
  }
...
If = (function() {
  function If(arg) {
    this.cond = arg.cond, this.st = arg.st, this.sf = arg.sf;
    assertType(this.cond, Cond, Boolean);
    assertType(this.st, If, While, Seq, Assign, Skip, Break, Continue, Exit);
    assertType(this.sf, If, While, Seq, Assign, Skip, Break, Continue, Exit);
  }
...
While = (function() {
  function While(arg) {
    this.cond = arg.cond, this.body = arg.body;
    assertType(this.cond, Cond, Boolean);
...
Expr = (function() {
  function Expr(arg) {
    this.e1 = arg.e1, this.op = arg.op, this.e2 = arg.e2;
    assertType(this.e1, Expr, Number, ValOf);
    assertType(this.op, '+', '-', '*', '/');
    assertType(this.e2, Expr, Number, ValOf);
  }
...
var Assign, Branch, Break, Cond, Continue, Exit, Expr, If, Loop, MAX_N_STATES, Save, Seq, Skip, ValOf, While, clone, evaluate, finalState, func, isa, ref, trans,
  slice = [].slice;

ref = __webpack_require__(0), Assign = ref.Assign, Seq = ref.Seq, If = ref.If, While = ref.While, ValOf = ref.ValOf, Expr = ref.Expr, Cond = ref.Cond, Save = ref.Save, Branch = ref.Branch, Loop = ref.Loop, Skip = ref.Skip, Break = ref.Break, Continue = ref.Continue, Exit = ref.Exit;
...
trans = function(arg) {
  var b, body, c, comm, cond, f, head, inclusive, indexOfWhile, loopAgain, m, n, n1, n2, newMem, ref1, remaining, s, sTail, stmtF, stmtT, tail, v, whileStmt;
  c = arg.c, s = arg.s, m = arg.m;
  head = c[0], tail = 2 <= c.length ? slice.call(c, 1) : [];
  if ((ref1 = typeof head) === 'number' || ref1 === 'boolean') {
    return {
      c: tail,
      s: [head].concat(slice.call(s)),
      m: m
    };
  }
  ...
  if (head instanceof Expr || head instanceof Cond) {
    return {
      c: [head.e1, head.e2, head.op].concat(slice.call(tail)),
      s: s,
      m: m
    };
  }
  if (isa(head, 'iop') || isa(head, 'bop')) {
    n2 = s[0], n1 = s[1], sTail = 3 <= s.length ? slice.call(s, 2) : [];
    f = func[head];
    n = f(n1, n2);
    return {
      c: tail,
      s: [n].concat(slice.call(sTail)),
      m: m
    };
  }
  ...
  if (head instanceof Assign) {
    return {
      c: [head.value, new Save].concat(slice.call(tail)),
      s: [head["var"]].concat(slice.call(s)),
      m: m
    };
  }
  if (head instanceof Save) {
    n = s[0], v = s[1], sTail = 3 <= s.length ? slice.call(s, 2) : [];
    newMem = clone(m);
    newMem[v] = n;
    return {
      c: tail,
  ...
  if (head instanceof If) {
    return {
      c: [head.cond, new Branch].concat(slice.call(tail)),
      s: [head.st, head.sf].concat(slice.call(s)),
      m: m
    };
  }
  if (head instanceof Branch) {
    b = s[0], stmtT = s[1], stmtF = s[2], sTail = 4 <= s.length ? slice.call(s, 3) : [];
    comm = b ? stmtT : stmtF;
    return {
      c: [comm].concat(slice.call(tail)),
      s: sTail,
      m: m
    };
  }
  if (head instanceof Seq) {
    return {
      c: [head.s1, head.s2].concat(slice.call(tail)),
      s: s,
      m: m
    };
  }
  if (head instanceof While) {
    return {
      c: [head.cond, new Loop].concat(slice.call(tail)),
      s: [head.cond, head.body].concat(slice.call(s)),
      m: m
    };
  }
  if (head instanceof Loop) {
    ...
    if (loopAgain) {
      ...
      return {
        c: [body, whileStmt].concat(slice.call(tail)),
        s: sTail,
        m: m
      };
    } else {
  ...
  indexOfWhile = function(inclusive) {
  ...
  if (head instanceof Continue) {
    remaining = c.slice(indexOfWhile(inclusive = false));
    return {
      c: remaining,
      s: s,
      m: m
  ...
  if (head instanceof Break) {
    remaining = c.slice(indexOfWhile(inclusive = true));
    return {
      c: remaining,
      s: s,
      m: m
...
evaluate = function(program) {
  ...
  while (current.c.length > 0) {
    if (current.c[0] instanceof Exit) {
      willExit = true;
    }
...
var grammar, nearley, parse;

nearley = __webpack_require__(6);

grammar = __webpack_require__(5);
...
(function() {
  var Assign, Break, Cond, Continue, Exit, Expr, If, Seq, Skip, ValOf, While, grammar, id, isKeyword, ref, strip, striphead;
  id = function(d) {
    return d[0];
  };
  ref = __webpack_require__(0), Assign = ref.Assign, Seq = ref.Seq, If = ref.If, While = ref.While, ValOf = ref.ValOf, Expr = ref.Expr, Cond = ref.Cond, Skip = ref.Skip, Break = ref.Break, Continue = ref.Continue, Exit = ref.Exit;
  isKeyword = function(x) {
    return x === 'true' || x === 'false' || x === 'if' || x === 'then' || x === 'else' || x === 'while' || x === 'do';
  };
  strip = function(d) {
    ...
    ParserRules: [
      {
        ...
        "name": "Stmt",
        "symbols": ["Stmt$string$2", "__", "Cond", "__", "Stmt$string$3", "__", "Body", "__", "Stmt$string$4", "__", "Body"],
        "postprocess": function(d) {
          return new If({
            cond: d[2],
            st: d[6],
        ...
        "name": "Stmt",
        "symbols": ["Stmt$string$5", "__", "Cond", "__", "Stmt$string$6", "__", "Body"],
        "postprocess": function(d) {
          return new While({
            cond: d[2],
            body: d[6]
        ...
        "name": "Var$ebnf$1",
        "symbols": ["Var$ebnf$1", /[\w_]/],
        "postprocess": function(d) {
          return d[0].concat([d[1]]);
        }
      }, {
        "name": "Var",
        "symbols": [/[a-z_]/, "Var$ebnf$1"],
        "postprocess": function(d) {
          return d[0] + d[1].join('');
        }
      }, {
...
var $, EXAMPLES, JSONFormatter, currStateBox, emptyInput, evalControl, evalOutput, evaluate, i, idx, jsonViewer, len, loadExample, parse, parseOutput, parseProgram, program, programInput, setupStates, showState;

$ = __webpack_require__(3);

JSONFormatter = __webpack_require__(4);

parse = __webpack_require__(2);

evaluate = __webpack_require__(1).evaluate;

EXAMPLES = ["a = 1;\nb = 7;\n\nif a < b then\n  min = a\nelse\n  min = b", "x = 3; sum = 0;\nwhile x > 0 do {\n  sum = sum + x;\n  x = x - 1\n}", "outer = 0;\nwhile outer < 2 do {\n  while true do {\n    break!\n  };\n  outer = outer + 1\n}"];

jsonViewer = function(obj) {
  var formatter, openedLevels;
  formatter = new JSONFormatter(obj, openedLevels = 2, {
...
showState = function(idx) {
  var currIdx;
  if (idx === 'last') {
    idx = this.states.length - 1;
  }
  ...
  if (+currStateBox.val() !== idx + 1) {
    currStateBox.val(idx + 1);
  }
  return evalOutput.removeClass('info error').html(jsonViewer(this.states[idx]));
};

currStateBox.change(function() {
  var nr;
  nr = currStateBox.val();
  return showState(+nr - 1);
});

$('#prev-state').click(function(e) {
  e.preventDefault();
  return showState('prev');
});

$('#next-state').click(function(e) {
  e.preventDefault();
  return showState('next');
});

setupStates = function(parsed) {
  var err;
  try {
    this.states = evaluate(parsed);
    evalControl.show();
    $('#nr-states').text(this.states.length);
    currStateBox.attr({
      max: this.states.length
    });
    return showState('last');
  } catch (error) {
    err = error;
    evalControl.hide();
    return evalOutput.removeClass('info').addClass('error').text(err.message);
  }
};

emptyInput = function() {
  parseOutput.removeClass('error').addClass('info').text('enter some code in the box above...');
  evalOutput.removeClass('error').addClass('info').text('...to play with its execution');
  return evalControl.hide();
};

parseProgram = function() {
  var err, parsed, program;
  ...
  try {
    ...
    parseOutput.removeClass('info error').html(jsonViewer(parsed));
    return setupStates(parsed);
  } catch (error) {
    err = error;
    parseOutput.removeClass('info').addClass('error').text(err.message.replace('nearley: ', ''));
    evalOutput.removeClass('error').addClass('info').text('fix syntax errors first');
    return evalControl.hide();
  }
};

programInput.keyup(function() {
  return parseProgram();
});
